export const assets = [

    //catjump
    {url: "./catjump/catjump1.png", name: "catjump1"},
    {url: "./catjump/catjump2.png", name: "catjump2"},
    {url: "./catjump/catjump3.png", name: "catjump3"},
    {url: "./catjump/catjump4.png", name: "catjump4"},
    {url: "./catjump/catjump5.png", name: "catjump5"},
    {url: "./catjump/catjump6.png", name: "catjump6"},
    {url: "./catjump/catjump7.png", name: "catjump7"},
    
    //lvlcomplete
    {url: "./UIlvlcomplete/metalPanel_blueCorner.png", name: "panel"},
    {url: "./UIlvlcomplete/UI-92.png", name: "completebar"},
    {url: "./UIlvlcomplete/UI-98.png", name: "greenbutton"},
    {url: "./UIlvlcomplete/UI-19.png", name: "exitbutton"},
    {url: "./UIlvlcomplete/UI-04.png", name: "textbox"},
    {url: "./UIlvlcomplete/lvlcompleteCat1.png", name: "lvlcompletecat1"},
    {url: "./UIlvlcomplete/lvlcompleteCat2.png", name: "lvlcompletecat2"},
    {url: "./UIlvlcomplete/lvlcompleteCat3.png", name: "lvlcompletecat3"},
    {url: "./UIlvlcomplete/lvlcompleteCat4.png", name: "lvlcompletecat4"},
    
    //bground
    {url: "./back.png", name: "backgroundimg"},

    //platform
    {url: "./platform-long.png", name: "longplatform"},

    //Bushes
    {url:"./bush.png", name: "bush"},

    //house
    {url:"./house.png", name: "house"},

    //tree
    {url:"./tree.png", name: "tree"},

    //UIpause
    {url: "./UIpause/UI-8.png", name: "cartelpause"},
    {url: "./UIpause/glassPanel_corners.png", name: "pausewindow"},
    {url: "./UIpause/musicOn.png", name: "musicOn"},
    {url: "./UIpause/forward.png", name: "play"},
    {url: "./UIpause/forwardon.png", name: "playon"},
    {url: "./UIpause/musicOff.png", name: "musicOff"},

    //runcat
    {url: "./runcat/runcat1.png", name: "runcat1"},
    {url: "./runcat/runcat2.png", name: "runcat2"},
    {url: "./runcat/runcat3.png", name: "runcat3"},
    {url: "./runcat/runcat4.png", name: "runcat4"},
    {url: "./runcat/runcat5.png", name: "runcat5"},
    {url: "./runcat/runcat6.png", name: "runcat6"},
    {url: "./runcat/runcat7.png", name: "runcat7"},
    {url: "./runcat/runcat8.png", name: "runcat8"},
    
    //standcat
    {url: "./standcat/standcat1.png", name: "standcat1"},
    {url: "./standcat/standcat2.png", name: "standcat2"},
    {url: "./standcat/standcat3.png", name: "standcat3"},
    {url: "./standcat/standcat4.png", name: "standcat4"},

    //walkcat
    {url: "./walkcat/walkcat1.png", name: "walkcat1"},
    {url: "./walkcat/walkcat2.png", name: "walkcat2"},
    {url: "./walkcat/walkcat3.png", name: "walkcat3"},
    {url: "./walkcat/walkcat4.png", name: "walkcat4"},
    {url: "./walkcat/walkcat5.png", name: "walkcat5"},
    {url: "./walkcat/walkcat6.png", name: "walkcat6"},
    {url: "./walkcat/walkcat7.png", name: "walkcat7"},
    {url: "./walkcat/walkcat8.png", name: "walkcat8"},


]

